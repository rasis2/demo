...
import {
  ...
  Palette, LayoutGrid, Upload, Image as ImageIcon, X, Globe, Type,
...
const STRINGS = {
  ...
  pdfBusy: { ms: "Menyediakan PDF…", en: "Preparing PDF…", zh: "正在生成PDF…", ta: "PDF தயாராகிறது…" },
  pdfPrintHint: {
    ms: "Sila pilih \"Save as PDF\" dalam dialog cetak untuk muat turun.",
    en: "Please choose \"Save as PDF\" in the print dialog to download.",
    zh: "请在打印对话框中选择“另存为PDF”下载。",
    ta: "பதிவிறக்க அச்சு உரையாடலில் \"Save as PDF\" என்பதை தேர்ந்தெடுக்கவும்.",
  },
  ...
  fontHeading: { ms: "Jenis font", en: "Font type", zh: "字体类型", ta: "எழுத்துரு" },
  fontSerif: { ms: "Klasik (Serif)", en: "Classic (Serif)", zh: "经典（衬线）", ta: "கிளாசிக் (செரிஃப்)" },
  fontSans: { ms: "Moden (Sans)", en: "Modern (Sans)", zh: "现代（无衬线）", ta: "நவீன (சான்ஸ்)" },
  fontElegant: { ms: "Elegan", en: "Elegant", zh: "优雅", ta: "நேர்த்தியான" },
...
/* ================= fonts ================= */
const FONTS = [
  { id: "serif", labelKey: "fontSerif", stack: "Georgia, 'Times New Roman', 'Noto Serif Tamil', 'Microsoft YaHei', serif" },
  { id: "sans", labelKey: "fontSans", stack: "'Segoe UI', Arial, Helvetica, 'Noto Sans Tamil', 'Microsoft YaHei', sans-serif" },
  { id: "elegant", labelKey: "fontElegant", stack: "'Trebuchet MS', 'Gill Sans', 'Verdana', 'Noto Sans Tamil', 'Microsoft YaHei', sans-serif" },
];
...
const captureResume = async (el) => {
  try {
    const canvas = await toCanvas(el, { pixelRatio: 2, backgroundColor: "#ffffff", cacheBust: true, pseudoElements: true });
    if (!isBlankCanvas(canvas)) return canvas;
  } catch (e) {
    /* blank or failed render → null so caller falls back to native print */
  }
  return null;
};

const nativePrintResume = (src) =>
  new Promise((resolve, reject) => {
    if (!src) return resolve();
    const iframe = document.createElement("iframe");
    iframe.className = "pdf-print-frame";
    iframe.style.cssText = "position:fixed;left:-100000px;top:0;width:8.5in;height:11in;border:0;";
    ...
    try {
      ...
      doc.close();
      const head = doc.head;
      document.querySelectorAll("link[rel='stylesheet'], style").forEach((el) => head.appendChild(el.cloneNode(true)));
      const style = doc.createElement("style");
      style.textContent =
        "@page { size: A4; margin: 0; }" +
        "html, body { margin: 0; padding: 0; background: #ffffff; }" +
        ".print-body { zoom: 0.9727; }" +
        ".print-body > div { box-shadow: none !important; margin-left: auto !important; margin-right: auto !important; }";
      head.appendChild(style);
      const body = doc.createElement("div");
      body.className = "print-body";
      const clone = src.cloneNode(true);
      while (clone.firstElementChild) body.appendChild(clone.firstElementChild);
      ...
      const finish = () => {
        ...
        win.onafterprint = () => { try { iframe.remove(); } catch (e) {} resolve(); };
        win.focus();
        win.print();
      };
      Promise.all([
        ...[...head.querySelectorAll("link[rel='stylesheet']")].map((l) =>
          l.sheet ? Promise.resolve() : new Promise((res) => { l.addEventListener("load", res); l.addEventListener("error", res); })
        ),
        ...[...body.querySelectorAll("img")].map((img) =>
          img.complete ? Promise.resolve() : new Promise((res) => { img.onload = res; img.onerror = res; })
        ),
      ])
        .then(() => {
          if (doc.fonts && doc.fonts.ready) return doc.fonts.ready.catch(() => {});
        })
        .then(() => setTimeout(finish, 150))
        .catch(() => setTimeout(finish, 150));
      setTimeout(() => { try { iframe.remove(); } catch (e) {} resolve(); }, 120000);
    } catch (err) {
...
export default function ResumeGenerator({ initialData, onChange }) {
  ...
  const getCaptureEl = () => {
    ...
    if (printArea && !printArea.classList.contains("w-14")) {
      const vis = printArea.querySelector(".preview-resume-wrap");
      if (vis && vis.firstElementChild) {
        const r = vis.firstElementChild.getBoundingClientRect();
        if (r.width > 200) return vis.firstElementChild;
      }
    }
  ...
  const handleSavePdf = async () => {
    ...
    try {
      let canvas = null;
      try {
        canvas = await captureResume(el);
      } catch (err) {
        console.error("Screenshot PDF failed, falling back to print", err);
      }
      if (canvas) {
        ...
        const slug = (info.name || "").trim().toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "") || "resume";
        pdf.save(`dear-resume-${slug}.pdf`);
        return;
      }
      alert(t("pdfPrintHint"));
      await nativePrintResume(captureRef.current);
    } catch (err) {
      console.error("Print PDF failed", err);
      alert(t("saveAsPdf") + " — " + (err && err.message ? err.message : "failed"));
    } finally {
      setPdfBusy(false);
    }
  };
  ...
  const data = { info, experience, education, certifications, projects, volunteering, customSections, skillList, languageList, hobbyList, awardList, showReferences, referencesText, skills, languages, awards, hobbies, themeId, layoutId, fontId, uiLang };
    ...
    <div className="h-screen flex flex-col overflow-hidden bg-gradient-to-b from-stone-100 to-stone-200 text-stone-900">
      ...
      <div className="no-print shrink-0 border-b border-stone-300 bg-white/90 backdrop-blur px-4 sm:px-6 py-3 flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          <a href="../index.html" title={t("backToPortal")} aria-label={t("backToPortal")} className="no-print flex items-center justify-center w-9 h-9 rounded-lg border border-stone-300 bg-white text-stone-500 hover:text-stone-900 hover:border-stone-400 shrink-0">
            <ArrowLeft size={16} />
          </a>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <label className="flex items-center gap-1.5 border border-stone-300 rounded-md bg-white pr-1 cursor-pointer" title={t("languages")}>
            <Globe size={13} className="ml-2 text-stone-400" />
            <select value={uiLang} onChange={(e) => setUiLang(e.target.value)}
              className="text-xs py-1.5 pr-1 pl-0.5 bg-transparent text-stone-700 outline-none cursor-pointer"
          ...
          <button onClick={handleSavePdf} disabled={pdfBusy} className="flex items-center gap-2 rounded-md px-4 py-2 text-sm font-medium text-white hover:opacity-90 disabled:opacity-60 disabled:cursor-wait" style={{ background: theme.primary }}>
            <Download size={16} /> {pdfBusy ? t("pdfBusy") : t("saveAsPdf")}
          </button>
      ...
      <div className={`flex-1 min-h-0 ${isWide ? "overflow-hidden" : "overflow-y-auto"} flex ${isWide ? "flex-row" : "flex-col"} gap-4 p-4 sm:p-6 max-w-7xl mx-auto w-full`}>
        ...
        <div className={`no-print transition-all duration-200 ${!isWide ? "w-full" : formCollapsed ? "w-14 shrink-0 h-full" : "flex-1 min-w-0 h-full overflow-y-auto"}`}>
          ...
          <Section title={`${t("layoutHeading")} & ${t("themeHeading")}`} tooltip={t("tooltipLayoutTheme")}>
            ...
            <div className="mt-3">
              <div className="flex items-center gap-1.5 text-xs text-stone-500 mb-2"><Type size={13} /> {t("fontHeading")}</div>
              <div className="flex flex-wrap gap-2">
                {FONTS.map((f) => (
                  <button key={f.id} onClick={() => setFontId(f.id)}
                    className={`text-xs px-3 py-1.5 rounded-md border transition-colors ${fontId === f.id ? "bg-stone-900 text-white border-stone-900" : "bg-white border-stone-300 text-stone-600 hover:border-stone-500"}`}>
                    {t(f.labelKey)}
                  </button>
                ))}
              </div>
        ...
        <div className={`print-area transition-all duration-200 ${!isWide ? "w-full" : previewCollapsed ? "w-14 shrink-0 h-full" : "flex-1 min-w-0 h-full overflow-y-auto"}`}>
            {previewCollapsed ? (
              ...
              <>
                ...
                <div className="pb-4 pr-3 preview-resume-wrap">
                  <ResumeDocument data={data} theme={theme} layoutId={layoutId} t={t} fontStack={font.stack} />
                </div>
              </>
            )}
        </div>
      ...
      {/* Hidden full-width capture for PDF export */}
      <div ref={captureRef} aria-hidden="true" className="no-print pdf-capture" style={{ position: "fixed", left: "-10000px", top: 0, width: "8.5in", background: "#ffffff" }}>
        <ResumeDocument data={data} theme={theme} layoutId={layoutId} t={t} fontStack={font.stack} />
      </div>
      ...
      {donateOpen && (
        <div className="no-print fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-5" onClick={() => setDonateOpen(false)}>
          <div className="relative w-full max-w-sm rounded-2xl bg-white p-6 text-center shadow-2xl" role="dialog" aria-modal="true" onClick={(e) => e.stopPropagation()}>
            ...
            <button onClick={() => setQrZoom(true)} className="mx-auto block rounded-xl border-2 border-stone-200 hover:border-[#e8a13d] transition-colors cursor-zoom-in" title={t("donateHint")} aria-label={t("donateHint")}>
              <img src="qr.jpg" alt={t("donateTitle")} className="w-56 h-56 rounded-lg object-contain" />
            </button>
      ...
      {/* QR zoom lightbox */}
      {qrZoom && (
        <div className="no-print fixed inset-0 z-[60] flex items-center justify-center bg-black/85 p-6" onClick={() => setQrZoom(false)} role="dialog" aria-modal="true">
          <div className="relative max-w-full" onClick={(e) => e.stopPropagation()}>
            <img src="qr.jpg" alt={t("donateTitle")} className="max-w-[92vw] max-h-[85vh] rounded-2xl bg-white p-5 shadow-2xl object-contain" />
            <button onClick={() => setQrZoom(false)} className="absolute -top-4 -right-4 w-10 h-10 rounded-full bg-white text-stone-700 shadow-lg flex items-center justify-center hover:bg-stone-100" aria-label={t("closeLabel")}>
              <X size={18} />
            </button>
          </div>
        </div>
...
function Page({ children, fontStack }) {
  return (
    <div className="bg-white shadow-xl mx-auto w-full max-w-[8.5in] min-h-[11in] text-stone-900 overflow-hidden" style={{ fontFamily: fontStack || "Georgia, 'Times New Roman', serif" }}>
      {children}
    </div>
  );
}
...
function ResumeDocument({ data, theme, layoutId, t, fontStack }) {
  const { info } = data;
  ...
  if (layoutId === "sidebarLeft" || layoutId === "sidebarRight") {
    ...
    return <Page fontStack={fontStack}><div className="flex min-h-[11in]">{layoutId === "sidebarLeft" ? <>{sidebar}{main}</> : <>{main}{sidebar}</>}</div></Page>;
  }

  if (layoutId === "headerBand") {
    return (
      <Page fontStack={fontStack}>
